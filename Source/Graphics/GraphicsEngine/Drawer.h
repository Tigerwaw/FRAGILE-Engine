...
public:
	...
	void RenderMesh(const Mesh& aMesh, const std::vector<std::shared_ptr<Material>>& aMaterialList, float aLODHeuristic = 1.0f);
	void RenderMeshShadow(const Mesh& aMesh);
	void RenderMeshDebugPass(const Mesh& aMesh, const std::vector<std::shared_ptr<Material>>& aMaterialList, float aLODHeuristic = 1.0f);
